...
public class DepartmentRepository(AppDbContext dbContext) : IDepartmentRepository
{
    ...
    public async Task<IEnumerable<Department>> Get(bool trackChanges) =>
        await (!trackChanges 
            ? _dbContext.Departments.Include(e => e.Faculty).AsNoTracking() 
            : _dbContext.Departments.Include(e => e.Faculty)).ToListAsync();

    public async Task<Department?> GetById(Guid id, bool trackChanges) =>
        await (!trackChanges ?
            _dbContext.Departments.Include(e => e.Faculty).AsNoTracking() :
            _dbContext.Departments.Include(e => e.Faculty)).SingleOrDefaultAsync(e => e.Id == id);
